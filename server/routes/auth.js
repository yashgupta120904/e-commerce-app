...
authRouter.post("/tokenIsValid", async (req, res) => {
  try {
    const token = req.header("x-auth-token"); // Fixed: Added quotes
    if (!token) return res.json(false);
...
// Get user data - Fixed: Moved outside of tokenIsValid route
authRouter.get('/', auth, async (req, res) => {
  try {
    const user = await User.findById(req.user);
    res.json({ ...user._doc, token: req.token }); // Fixed: user._doc not user>_doc
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});
